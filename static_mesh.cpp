...
static_mesh::static_mesh(ID3D11Device* device, const wchar_t* obj_filename)
{
    ...
    while (fin)
    {
        ...
        else if (0 == wcscmp(command, L"vt"))
        {
            float u, v;
            fin >> u >> v;
            texcoords.push_back({ u,texture_correction-v });
            fin.ignore(1024, L'\n');
        }
    ...
    for (material& material : materials)
    {
        ...
        {
            make_dummy_texture(device, material.shader_resource_view[0].GetAddressOf(), 0xFFFFFFFF, 16);
            make_dummy_texture(device, material.shader_resource_view[1].GetAddressOf(), 0xFFFF7F7F, 16);
        }
        else
        {
            for (int texture_type = 0; texture_type < 2; texture_type++)
            {
                if (material.texture_filename[texture_type].size() > 0)
                {
                    load_texture_from_file(device, material.texture_filename[texture_type].c_str(), material.shader_resource_view[texture_type].GetAddressOf(), &texture2d_desc);
                }
            }
        }
    }
...
HRESULT static_mesh::make_dummy_texture(ID3D11Device* device, ID3D11ShaderResourceView** shader_resource_view, DWORD value, UINT dimension)
{
    HRESULT hr{ S_OK };
